...
locals {
   get_value = "${lookup(var.id_change, var.instance_type)}"
}
...
data "null_data_source" "recovery" {
    inputs = {
        i3_2xlarge = "0"
        m4_xlarge  = "${var.count}"
        m4_2xlarge = "${var.count}"
        i3_4xlarge = "0"
        i3_8xlarge = "0"
        i3en_6xlarge = "0"
        i3en_2xlarge = "0"
        m5a_xlarge = "${var.count}"
    }
}
...
#  subnet_id               = "${element(split(",", replace(replace(replace(replace(replace(replace(join(",", data.aws_subnet_ids.private.ids), "/subnet-3bf7bf62,/", ""), "/subnet-18036933,/", ""), "/subnet-d609488f,/", ""), "/subnet-c706e7fa,/", ""), "/subnet-100a722d,/", ""), "/subnet-3d809c07,/", "")), count.index)}"
  subnet_id               = "${element(split(",", replace(replace(replace(replace(join(",", data.aws_subnet_ids.private.ids), "/subnet-18036933,/", ""), "/subnet-3d809c07,/", ""), "/subnet-c706e7fa,/", ""), "/subnet-100a722d,/", "")), count.index)}"
...
resource "aws_cloudwatch_metric_alarm" "recovery" {
    ...
    count 	 	= "${lookup(data.null_data_source.recovery.inputs, local.get_value)}"
...
resource "aws_cloudwatch_metric_alarm" "disk_usage_data" {
    alarm_name          = "${element(aws_instance.ds-operations.*.id, count.index)}-disk_usage_data"
    comparison_operator = "GreaterThanThreshold"
    evaluation_periods  = "1"
    metric_name         = "DiskSpaceUtilization"
    namespace           = "System/Linux"
    ...
    statistic           = "Average"
    threshold           = "70.0"
    alarm_description   = "Data filesystem over 60%"
    count               = "${var.count}"
    alarm_actions      	= ["${aws_sns_topic.health_updates.arn}"]
    ok_actions         	= ["${aws_sns_topic.health_updates.arn}"]
      dimensions {
...
resource "aws_cloudwatch_metric_alarm" "disk_usage_data00" {
    alarm_name          = "${element(aws_instance.ds-operations.*.id, count.index)}-disk_usage_data00"
    comparison_operator = "GreaterThanThreshold"
    evaluation_periods  = "1"
    metric_name         = "DiskSpaceUtilization"
    namespace           = "System/Linux"
    ...
    ok_actions          = ["${aws_sns_topic.health_updates.arn}"]
      dimensions {
         Filesystem = "/dev/mapper/DatastaxVG00-DatastaxLV00"
         InstanceId = "${element(aws_instance.ds-operations.*.id, count.index)}"
         MountPath = "/mnt/data00"
    }
}

resource "aws_cloudwatch_metric_alarm" "disk_usage_data01" {
    alarm_name          = "${element(aws_instance.ds-operations.*.id, count.index)}-disk_usage_data01"
    comparison_operator = "GreaterThanThreshold"
    evaluation_periods  = "1"
    metric_name         = "DiskSpaceUtilization"
    namespace           = "System/Linux"
    ...
    ok_actions          = ["${aws_sns_topic.health_updates.arn}"]
      dimensions {
         Filesystem = "/dev/mapper/DatastaxVG01-DatastaxLV01"
         InstanceId = "${element(aws_instance.ds-operations.*.id, count.index)}"
